...
###Major by Gender
![Major By Gender](https://github.com/TZstatsADS/cycle1-4/tree/master/figs/Rplot.jpeg)
```{r, echo=FALSE}
...
![Edu Level](/blob/master/figs/Rplot02.jpeg)
```{r, echo=FALSE}
...
![Employment Status](/blob/master/figs/Rplot01.jpeg)
```{r, echo=FALSE}
...
![Work Sector](/blob/master/figs/Rplot03.jpeg)
```{r, echo=FALSE}
...
###Wage vs Major
![Wage by Major](/blob/master/figs/Rplot04.jpeg)
```{r, echo=FALSE}
...
![Major By Gender](/blob/master/figs/Rplot05.jpeg)
```{r}
...
```
The Chi-squared test revealed a statistically signicant assocation (p<0) between wage and education level in the field of statistics. So, we should keep purusing those higher degrees not only for the advancement of personal knowledge, but for higher wages aswell.

So now that we have established that we will have a job, we will make some money, where will we end up? We conclude with a spatial examination of where we will live.

##Where are we located?
###Mapping Just Statistics Majors
![Location](/blob/master/figs/ONLY_STAT.png)
```{r}
...
![Location Pt 2](/blob/master/figs/zoom_out.png)
```{r}
state_choropleth(junk2,
